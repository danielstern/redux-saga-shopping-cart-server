...
nativeObject = YAML.load('database.yml', (database) => {
    ...
    app.post("/items", (req, res) => {
        const existingItem = database.items.find(item => item.name === req.body.name);
        if (existingItem) {
            res
                .status(422)
                .json({error: "An Item with same name already exists"});
        } else {
            const id = ID();
            database.items.push({
                ...req.body,
            ...
            res
                ...
                .json({
                    ...
                    item: database.items.find(i => i.id === id),
                });
        }
    });
    ...
    app.get("/items/:ids?", (req, res) => {
        ...
        if (req.params.ids) {
            ...
            items = ids.map(id => database.items.find(item => item.id === id));
        } else {
